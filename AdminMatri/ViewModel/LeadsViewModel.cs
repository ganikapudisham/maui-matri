...
public partial class LeadsViewModel : CommunityToolkit.Mvvm.ComponentModel.ObservableObject
{
    ...
    private Command<SfComboBox> onSelectionChangedCommand;

    public Command<SfComboBox> OnSelectionChangedCommand
    {
        get { return onSelectionChangedCommand; }
        set { onSelectionChangedCommand = value; OnPropertyChanged(nameof(OnSelectionChangedCommand)); }
    }
    ...
    public LeadsViewModel(IServiceManager serviceManager, IPopupService popupService)
    {
        ...
        IsBusy = false;
        this.OnSelectionChangedCommand = new Command<SfComboBox>(OnSelectionChangedCommandMethod);
    }
    ...
    public async Task<bool> GetLeads()
    {
        ...
        try
        {
            var dbLeads = await _serviceManager.GetLeads(token, SelectedGroupName.Name);
    ...
    private void OnSelectionChangedCommandMethod(SfComboBox comboBox1)
    {
        IsBusy = true;
        var selectedItem = comboBox1.SelectedItem as Master;
        SelectedGroupName = selectedItem;

        Device.BeginInvokeOnMainThread(async () =>
        {
            var leads = await _serviceManager.GetLeads("", selectedItem.Name.Replace(' ', '_'));
            Leads.Clear();
