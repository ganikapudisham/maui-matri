...
{
    public partial class RequestsSentViewModel : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IQueryAttributable
    {
        ...
        public RequestsSentViewModel(IServiceManager serviceManager)
        {
        ...
        public async Task<bool> GetProfiles(int pageNumber, int pageSize)
        {
            ...
            try
            {
                ...
                var dbProfilesWithPaging = await _serviceManager.GetRequestSent(new Guid(token), pageNumber, pageSize);
        ...
        {
            if (obj != null && obj is RequestsSentViewModel)
            {
                var item = (RequestsSentViewModel)obj;
        ...
        {
            IsBusy = true;
            if (obj != null && obj is RequestSent)
            {
                var item = (RequestSent)obj;

                var sessionToken = await SecureStorage.GetAsync("Token");
                var targetProfileId = item.ReceiverId;
